...
from asyncstdlib import enumerate # type: ignore
...
def get_tokens(signature: Signature, RaydiumLPV4: Pubkey) -> None:
  ...
  for instruction in filtred_instuctions:
    ...
    if str(tokens[0]) == SOL_Token and get_liquidity(tokens[1]):
      print_table(tokens)
      print(f"True, https://solscan.io/tx/{signature}")
    elif str(tokens[1]) == SOL_Token and get_liquidity(tokens[0]):
      print_table(tokens)
      print(f"True, https://solscan.io/tx/{signature}")
...
def get_liquidity(add):
  url = f"https://api.dexscreener.com/latest/dex/tokens/{add}"
  try:
    resp = httpx.get(url)
